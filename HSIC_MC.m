function  [Z,P,Ks, Kt,KY_test,KY_valid]=HSIC_MC(Xs_train,Xt_train,Xtest,Xvalid,Graph_Xt_train_num,Kernel)
theta=1;        %%MC系数
tau=1;         %%MMD系数
alpha=0.3;      %%Z梯度下降系数
mu=0.1;         %%R更新系数
...
% Y2 = Construct_Y(target_label_test,length(target_label_test)); 
   ...
   switch Kernel 
       ...
       KY_valid=gram(X',Xvalid',Kernel,kervar1,kervar2);
       KY_valid = KY_valid./repmat(sqrt(sum(KY_valid.^2)),[size(KY_valid,1) 1]); 
